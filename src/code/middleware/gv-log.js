...
export default loggingMetadata => store => next => action => {
  ...
  if (!listeningToITSData && isLoggingEnabled) {
    ...
    if (userQueryString) {
      ...
      firebase.database().ref(userQueryString + "/itsData").on('value', function(snapshot) {
        ...
        const message =
          {
            ...
            session: session,
            time: Date.now(),
            activity: AuthoringUtils.getChallengeId(nextState.authoring, nextState.routeSpec),
