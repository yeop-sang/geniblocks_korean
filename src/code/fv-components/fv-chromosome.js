...
import GeneLabelView from '../components/gene-label';
import AlleleView from '../components/allele';
...
const FVChromosomeView = ({chromosome, org, chromosomeName, side, userChangeableGenes = [], visibleGenes = [], hiddenAlleles = [], small = false, editable = true, selected = false, onAlleleChange, onChromosomeSelected, showLabels = true, showAlleles = false, labelsOnRight = true, orgName, displayStyle = {}}) => {
    ...
    <div className="geniblocks chromosome-container" onClick={ handleSelect } >
      <div className={ containerClass }>
        <div className="chromosome-allele-container" id={chromId} style={displayStyle}>
          <FVChromosomeImageView small={small} empty={empty} bold={selected} yChromosome={yChromosome}/>
          { allelesContainer }
        </div>
        { labelsContainer }
      </div>
...
export default FVChromosomeView;
