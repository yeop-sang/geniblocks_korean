 ...
 */
const FVStableView = ({orgs, idPrefix='organism-', height=148, tightenRows=0, tightenColumns=0, onClick}) => {
  ...
  let stableDrakeHeight = height/2,
      stableDrakeStyle = { margin: `${-tightenRows/2}px ${-tightenColumns/2}px`, height: stableDrakeHeight},
      stableDrakeViews = orgs.map((org, index) => {
        return <StableDrakeView org={org} id={idPrefix + index} index={index} key={index}
                                height={stableDrakeHeight} style={stableDrakeStyle} onClick={handleClick} />;
      });

  return (
    <div className="geniblocks fv-stable">
      <div className="stable-text">{"Stable Count:" + '\xa0\xa0\xa0\xa0\xa0\xa0\xa0\xa0\xa0\xa0\xa0\xa0\xa0\xa0\xa0' + orgs.length}</div>
